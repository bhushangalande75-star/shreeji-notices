...
from database import (save_batch, get_batches, get_batch_notices, update_payment,
                      ...
                      get_analytics,
                      init_agm_tables,
                      create_agm_meeting, get_agm_meetings, get_agm_meeting,
                      update_agm_status, save_agm_transcript, save_agm_minutes,
                      delete_agm_meeting, record_agm_join, record_agm_leave,
...
# ══════════════════════════════════════════════════════════════════════════════
# VIRTUAL AGM / SGM — All routes (100% free: Jitsi + Groq Whisper + Groq LLaMA)
# ══════════════════════════════════════════════════════════════════════════════

def _transcribe_with_groq(audio_bytes, filename="meeting.webm"):
    """Transcribe meeting audio using Groq Whisper API (free tier: 28,800s/day)."""
    if not GROQ_API_KEY:
...
Generate complete formal Minutes of Meeting including:
1. Header with society name placeholder [SOCIETY NAME]
2. Meeting details (type, date, time, platform)
3. Confirmation of digital notice served (valid under 2019 Amendment)
4. Attendance and quorum confirmation
5. Agenda items discussed
6. All resolutions with vote counts
7. Any other business
8. Signature blocks for Chairman and Secretary
"""
...
@login_required
def agm_list():
    meetings = get_agm_meetings(session["society_id"])
...
def agm_detail(mid):
    ...
    return render_template("agm_detail.html", meeting=meeting,
                           attendance=attendance, votes=votes)
...
@login_required
def agm_start(mid):
    if not get_agm_meeting(mid, session["society_id"]):
...
def agm_vote_close(vid):
    close_agm_vote(vid)
    return jsonify({"success": True, "results": get_vote_results(vid)})
...
def agm_transcribe(mid):
    ...
    try:
        transcript = _transcribe_with_groq(audio_bytes,
                                           request.files["audio"].filename or "meeting.webm")
...
def agm_save_transcript(mid):
    if not get_agm_meeting(mid, session["society_id"]):
        return jsonify({"success": False}), 404
    save_agm_transcript(mid, request.json.get("transcript", "").strip())
    return jsonify({"success": True})
...
def agm_generate_minutes(mid):
    ...
    try:
        attendance = get_agm_attendance(mid)
        votes      = get_agm_votes(mid)
        minutes    = _generate_agm_minutes(meeting, attendance, votes,
...
def agm_save_minutes_route(mid):
    ...
    if text and request.json.get("index_kb"):
        try:
            ...
            doc_name = f"AGM Minutes - {meeting['title']}"
